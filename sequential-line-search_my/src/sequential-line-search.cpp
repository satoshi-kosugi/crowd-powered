...
void sequential_line_search::SequentialLineSearchOptimizer::AddLineSearchResult(const VectorXd& x_chosen,
                                                                                   const VectorXd& end_0,
                                                                                   const VectorXd& end_1)
{
    const int num_dims                 = GetMaximizer().size();
    const int num_map_estimation_iters = 100;
#ifdef SEQUENTIAL_LINE_SEARCH_USE_PARALLELIZED_MULTI_START_SEARCH
    ...
    // Update the data
    // m_data->AddNewPoints(x_chosen, {x_prev_max, x_prev_ei}, true);
    m_data->AddNewPoints(x_chosen, {end_0, end_1}, true);
...
void sequential_line_search::SequentialLineSearchOptimizer::PrintTest() const
{
